...
helm install argocd argo/argo-cd --version 10.0.0 --namespace argocd --create-namespace --set server.service.type=NodePort --set server.service.nodePortHttps=30179 --set 'configs.cm.kustomize\.buildOptions=--enable-helm' --set 'configs.cm.application\.sync\.impersonation\.enabled=true' | tee -a  ~/.status.log 

# Install Sealed Secrets
# helm repo add sealed-secrets https://bitnami.github.io/sealed-secrets | tee -a ~/.status.log
